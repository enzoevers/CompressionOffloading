...
bool RecursiveMkdir(const char *const pDirname) {
    ...
    if (!IsAbsolutePath(pDirname)) {
        ...
        // (dirnameLength + 1) to include the null terminator
        memcpy(&targetPath[0], pDirname, sizeof(char) * (dirnameLength + 1));
    }
...
bool _SkipRootPathOfPath(const char *pPath) {
    ...
    const size_t pathLenght = strnlen(pPath, MAX_PATH_LENGTH_SIZE + 1);
    if (pathLenght == 0 || pathLenght > MAX_PATH_LENGTH_SIZE) {
        return false;
    }
